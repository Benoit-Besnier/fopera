...
from myia.externals.parsing.game.turn import Turn
from myia.externals.parsing.parser.read_question import Question, Type
...
def game_over(role: int) -> bool:
    with open(f"./{role}/infos.txt", 'r') as f_handler:
        data = f_handler.read()
    if not data:
        return False
