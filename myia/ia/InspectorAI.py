...
class InspectorAI(AI):
    ...
    @staticmethod
    def __get_color_options(option_available: List[Tuile.Color]) -> List[str]:
        color_option: List[str] = []
        match_color_enum: List[str] = ['rose', 'gris', 'rouge', 'marron', 'bleu', 'violet', 'blanc', 'noir']

        for option in option_available:
            color_option.append(match_color_enum[option.value - 1])
        return color_option
